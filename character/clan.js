...
const skills = {
  ...
  clanshixi: {
    ...
    onChooseToUse(event2) {
      if (!game.online && !event2.clanshixi) {
        ...
        const list = Object.entries(storage).filter(([suit, name]) => {
          ...
          if (!lib.skill.clanshixi.filterx(card, player2) || !event2.filterCard(card, player2, event2)) {
            return false;
          }
          return true;
        });
    ...
    filterx(card, player2) {
      if (get.type(card, null, false) !== "trick") {
        return false;
      }
      const info = get.info(card, player2);
    ...
    subSkill: {
      mark: {
        init(player2, skill) {
          const history = player2.getAllHistory("useCard", (evt) => get.suit(evt.card) !== "none" && lib.skill.clanshixi.filterx(evt.card, player2));
          if (history.length) {
        ...
        filter(event2, player2) {
          ...
          if (!lib.skill.clanshixi.filterx(card, player2)) {
            return false;
          }
          const suit = get.suit(card);
          if (suit === "none") {
  ...
  clandongxu: {
    ...
    subSkill: {
      yang: {
        ...
        enable: "chooseToUse",
        hiddenCard(player2, name) {
          return name == "sha" && !player2.storage.clandongxu && player2.hasCards("e");
        },
        filter(event2, player2) {
          return event2.filterCard(get.autoViewAs({ name: "sha", isCard: true }), player2, event2) && !player2.storage.clandongxu && player2.hasCards("e");
        },
        ...
        ai: {
          ...
          skillTagFilter(player2, tag, arg) {
            ...
            if (tag == "respondSha" && (player2.storage.clandongxu || !player2.hasCards("e"))) {
              return false;
            }
          }
        }
  ...
  clanyangji: {
    ...
    filter(event2, player2, name) {
      if (name !== "phaseZhunbeiBegin" && !game.hasGlobalHistory("changeHp", (evt) => evt.player === player2 && evt.changedHp !== 0 && evt.getParent(event2.name) === event2)) {
        return false;
      }
      return true;
    },
  ...
  clanlianhe: {
    ...
    async content(event2, trigger, player2) {
      ...
      player2.addSkill(event2.name + "_effect");
      player2.markAuto(event2.name + "_effect", targets);
    },
    subSkill: {
      effect: {
        audio: "clanlianhe",
