...
const skills = {
  ...
  youtan: {
    ...
    async content(event, trigger, player) {
      ...
      if (suits?.length) {
        ...
        player.addTip(
          event.name,
          `忧叹${player.getStorage(event.name).sort((a, b) => lib.suit.indexOf(b) - lib.suit.indexOf(a)).map((i) => get.translation(i)).join("")}`
        );
      }
    },
    mod: {
  ...
  zhancai: {
    ...
    async content(event, trigger, player) {
      ...
      const result = await player.chooseButton(["展才：移去一个花色", [list.map((suit) => ["", "", `lukai_${suit}`]), "vcard"]], true).set("ai", (button) => {
      ...
      if (result?.bool && result.links?.length) {
        ...
        player.addTip(
          skill,
          `忧叹${player.getStorage(skill).map((i) => get.translation(i)).join("")}`
        );
      }
    },
    ai: {
