    ...
    async cost(event, trigger, player) {
      const result = await player.chooseButtonTarget({
        ...
        filterButton(button) {
          ...
          if (button.link === "give") {
            return game.hasPlayer((current) => current != player2 && current.countCards("he") >= 1 && (!chosen[0] || trigger2.name == "phaseZhunbei"));
          }
          return game.hasPlayer((current) => current != player2 && current.countCards("hej") && (!chosen[1] || trigger2.name == "phaseZhunbei"));
        },
        filterTarget(card, player2, target) {
          ...
          if (link === "give") {
            return target.countCards("he") >= 1 && (!chosen[0] || trigger2.name == "phaseZhunbei");
          }
          return target.countCards("hej") && (!chosen[1] || trigger2.name == "phaseZhunbei");
        },
        ai1(button) {
          const player2 = get.player();
          ...
          const trigger2 = get.event().getTrigger();
          if (link == "gain") {
            if (trigger2.name == "phaseZhunbei" && !player2.hasShan() && !game.hasPlayer((current) => current != player2 && current.countCards("he") >= 1 && get.attitude(player2, current) > 0)) {
              return 2.5;
            }
            return 1.5;
          } else {
...
const translates = {
  ...
  dc_jiben: "吉本",
  xunli: "询疠",
  xunli_info: "锁定技。①当有黑色牌因弃置而进入弃牌堆后，若X大于0，则你将其中的随机X张牌置于武将牌上作为“疠”（X=min(这些牌的数量，9-Y)，Y=你的“疠”数）。②出牌阶段开始时，你可以用任意张黑色手牌交换等量的“疠”。",
  zhishi: "指誓",
